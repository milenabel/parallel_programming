...
int main(int argc, char *argv[]){
  ...
  for(i=0;i<N;i++) { wref[i] = 2.0*i/N; x[i] = 3.0*i/N; y[i] = 5.0*i/N; }
  
// Warmup instance is not timed
  vec3_ref(N, NREPS, &wref[0], &x[0], &y[0]);
  ...
  for(trial=0;trial<NTrials;trial++)
  {
   ...
   for (trial=0;trial<NTrials;trial++)
   {
    for(i=0;i<N;i++) { w[i] = 2.0*i/N; x[i] = 3.0*i/N; y[i] = 5.0*i/N; }
    tstart = omp_get_wtime();
    vec3_opt(N, NREPS, &w[0], &x[0], &y[0]);
    telapsed = omp_get_wtime()-tstart;
