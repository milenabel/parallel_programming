...
// matrix multiply kernel: C = A^T * B^T with 4-way unrolling along k
__global__ void mmtt_k4(const float *A, const float *B, float *C, int ds) {
...
int main() {
    ...
    for (int trial = 0; trial < 3; trial++) {
        ...
        // Launch kernel
        mmtt_k4<<<grid, block>>>(d_A, d_B, d_C, DSIZE);
